...
export default function AdminNotifications() {
  ...
  const handleNotificationClick = (n) => {
    ...
    if (n.booking_id?._id) {
      navigate(`/admin/bookings?id=${n.booking_id._id}`);
    }
  };
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <h2 className="font-display">Studio Notifications</h2>
          <p style={{ color: 'var(--white-40)', fontSize: '0.85rem' }}>Incoming bookings, cancellations, and client response logs.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="card-glass auth-card" style={{ maxWidth: 'none', padding: '48px', textAlign: 'center' }}>
          ...
          <h4 style={{ margin: 0 }}>No studio alerts</h4>
          <p style={{ color: 'var(--white-40)', fontSize: '0.9rem', marginTop: '6px' }}>All caught up! No notifications to display.</p>
        </div>
      ) : (
